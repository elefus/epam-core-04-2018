...
public class Task18 {
    ...
    public static void main(String[] args) {
        ...
        showMatrix(matrixWithoutRowsAndColomns(matrix));
    }
    ...
    private static int[][] deleteColomn(int[][] matrix, Set<Integer> set) {
    ...
    private static int[][] matrixWithoutRowsAndColomns(int[][] matrix) {
        int max = findMaxElement(matrix);

        Set<Integer> setRowsOfMax = new HashSet<>();
        Set<Integer> setColomnsOfMax = new HashSet<>();

        for (int row = matrix.length - 1; row >= 0; --row) {
            for (int col = matrix[row].length - 1; col >= 0; --col) {
                if (matrix[row][col] == max) {
                    setRowsOfMax.add(row);
                    setColomnsOfMax.add(col);
                }
            }
        }

        matrix = deleteRow(matrix, setRowsOfMax);
        matrix = deleteColomn(matrix, setColomnsOfMax);
        return matrix;
    }
