...
public class Task26analyzator implements Task26{
    @Override
    public Set<I2DPoint> analyze(Set<ISegment> segments) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            for (int j = i + 1; j < list.size(); j++) {
                Point2D intersectionPoint = getIntersectionPoint(list.get(i), list.get(j));
                if (intersectionPoint != null) {
                    segmentIntersections.add(intersectionPoint);
                }
            }
            addPoints(intersections, segmentIntersections);
        }
        return intersections.firstEntry().getValue();
    }

    private void addPoints(TreeMap<Double, Set<I2DPoint>> intersectionPoints, Set<I2DPoint> points) {
        for (Iterator<I2DPoint> iterator = points.iterator(); iterator.hasNext(); ) {
            I2DPoint point = iterator.next();
